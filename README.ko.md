...
설치 전에 동작만 먼저 보고 싶다면 클론해서 번들 CLI를 바로 돌려도 됩니다. 미리
깔아야 할 것이 없습니다:
...
빠른 경로에는 Node 18 이상이 필요합니다. 없어도 스킬은 동작합니다 — `curl`
(Windows에서는 PowerShell)로 내려가 에이전트가 같은 일을 처리합니다.
...
히어로 이미지, 빈 상태, 에러 페이지, 온보딩 아트, 플레이스홀더 벡터 모두 같은 6
단계를 거칩니다: 검색 → 사용자가 선택 → 저장 위치 제안 → 스타일시트에서 찾은 색상
...
`--color`에 넘기면 됩니다 — shadcn의 맨 HSL 채널(`214 92% 58%`), Tailwind v4의
...
전부이고, Node가 없으면 같은 일을 하는 `curl` 전용 경로로 내려갑니다.

**한 번에 파일 하나씩만 받습니다.** 설계가 그렇습니다. 대량 다운로드도, 로컬
미러링도 하지 않으며, unDraw 공식 다운로더가 넣는 `artist`·`copyright` 속성을
결과물에 보존합니다. 이유는 [라이선스](#라이선스)에 있습니다.
...
`--limit`의 상한은 50이고, 검색은 20페이지에서 멈추거나 새로운 결과가 없는 페이지가
나오는 즉시 멈춥니다. API 응답 구조가 바뀌어도 루프가 폭주하지 않습니다.

**`search`가 준 `media` URL을 그대로 넘기세요.** CDN 경로는 슬러그에서 유도할 수
없습니다 — 최근 일러스트는 `/illustration/` 아래, 오래된 것은 `/illustrations/`
아래에 있어서, 슬러그로 조립한 URL은 카탈로그 상당수에서 404가 납니다.
...
브라우저 전용 에디터라 의미 있게 감쌀 방법이 없습니다. Handcrafts도 공식 API가
없고, SVG가 배포마다 해시가 바뀌는 JS 번들 안에 인라인돼 있어서 — 추가는 가능하지만
깨지기 쉽습니다.
...
일러스트는 unDraw 자체 라이선스를 따르며, 이 도구를 쓰기 전에 읽어보셔야 합니다.
...
유사·경쟁 서비스를 만들기 위해 에셋을 모으거나 묶음으로 배포할 권리는 포함되지
않으며, 이 제한은 동의 없이 링크·임베드·스크랩·검색·다운로드하는 자동/수동 방식
모두에 적용된다는 뜻입니다.
...
인공지능·머신러닝 모델의 학습, 파인튜닝, 개발에 unDraw 에셋을 쓰는 것을 금지한다는
뜻입니다.
...
이 도구는 취득을 자동화하므로 제한 대상 범주에 들어갑니다. 사용자가 자기 프로젝트에
쓸 일러스트를 개별적으로 받는 용도 — 웹사이트의 다운로드 버튼이 하는 일과 같은 —
로만 쓴다는 전제 위에 공개했으며, 미러나 경쟁 카탈로그를 만들기 위한 것이 아닙니다.
대량 다운로드를 하지 않고, 에셋을 재배포하지 않으며, unDraw 공식 다운로더가 각 SVG에
써넣는 `artist`·`copyright` 속성을 보존합니다.
